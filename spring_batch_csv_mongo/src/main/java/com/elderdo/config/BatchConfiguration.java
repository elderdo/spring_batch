...
import com.elderdo.listener.JobCompletionNotificationListener;
import com.elderdo.model.UserDetail;
import com.elderdo.processor.UserItemProcessor;
...
public class BatchConfiguration {
	...
	public MongoItemWriter<User> writer(MongoTemplate mongoTemplate) {
		return new MongoItemWriterBuilder<User>().template(mongoTemplate).collection("user").build();
	}
	...
	public Step step1(FlatFileItemReader<UserDetail> itemReader, MongoItemWriter<User> itemWriter) throws Exception {
		...
		return this.stepBuilderFactory.get("step1").<UserDetail, User>chunk(5).reader(itemReader).processor(processor())
